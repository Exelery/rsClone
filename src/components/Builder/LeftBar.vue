  ...
  <div class="d-flex  flex-column flex-shrink-0 p-3 text-bg-dark left-bar" style="width: 280px;">
    ...
    <div class="dropdown">
      ...
      <ul class="dropdown-menu dropdown-menu-dark text-small shadow" aria-labelledby="dropdownUser1">
        <li><router-link class="dropdown-item" to="/user">{{ $t("builder.projects") }}</router-link></li>
        <li>
          <hr class="dropdown-divider">
        </li>
        <li><router-link class="dropdown-item" to="/">{{ $t("builder.main") }}</router-link></li>
      </ul>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getUserName() {
      ...
      await authStore.addUserParams()
      console.log(authStore.authUser)
      this.name = authStore.authUser.name
    }
  },
  created() {
